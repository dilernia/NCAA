...
my.boost <- caret::train(form = form, data = modelData, method = "gbm", 
                         distribution = ifelse(model == "Awin", 
                                               "bernoulli", "gaussian"),
                         trControl = ctrl, tuneGrid = expand.grid(n.trees = c(50, 100, 150, 200, 300), 
...
my.nnet <- caret::train(form, data = modelData, method = "nnet", 
                        trControl = ctrl, trace = FALSE, maxit = 1000,
...
my.nnet3 <- caret::train(form, data = modelData, method = 'mlpWeightDecayML', 
                  trControl = ctrl, trace = FALSE, maxit = 100,
                  tuneGrid = expand.grid(layer1 = c(5, 15),
                                         layer2 = c(0, 5, 15),
                                         layer3 = c(0, 5, 15),
                                         decay = c(0.01)),
                  linout = ifelse(model == "Awin", FALSE, TRUE))
...
my.nnetf <- caret::train(form, data = modelData, method = 'pcaNNet', 
                         trControl = ctrl, trace = FALSE, maxit = 5000,
                         tuneGrid = expand.grid(size = c(20),
                                                decay = c(0.01)),
                         linout = ifelse(model == "Awin", FALSE, TRUE))
