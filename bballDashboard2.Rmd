...
bracketDraw <- function(submission, seeds, teams, logo = NULL, lineColor = "black", 
  ...
  # Coordinates for drawing team names & probabilities
    ...
    dplyr::filter(x != xend) |> 
    dplyr::mutate(Xstart = case_when(Region %in% regions[3:4] ~ xend,
                                     TRUE ~ x), 
                  Y = y) |> 
  ...
  bracketGG <- empty.bracket +
    labData |> 
    dplyr::mutate(Y = Y + 0.005) |> 
    geom_text(mapping = aes(x = Xstart, y = Y,
                               label = TeamName, text = TeamName),
              size = rel(textSize), hjust = 0, vjust = 0,
              family = fontFamily) +
    geom_text(data = tibble(Region = regions,
                                  Xstart = c(rep(unique(r4Labs$Xstart)[1], 2),
                                             rep(unique(r3Labs$Xstart)[2], 2)),
                                  Y = rep(c(mean(unique(r4Labs$Y)[3:4]), 
                                            mean(unique(r4Labs$Y)[1:2])), times = 2)),
                    aes(x = Xstart, y = Y,
