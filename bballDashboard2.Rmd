...
bracketDraw <- function(submission, seeds, teams, logo = NULL, lineColor = "black", 
                        fontFamily = "Passion One", textSize = 6, regions = setNames(c("SOUTH", "EAST", "MIDWEST", "WEST"),
                        c("X", "W", "Y", "Z")), retData = FALSE) {
  ...
  bracketGG <- empty.bracket + # Adding regions 1 and 2 games
    ...
    geom_text(data = tibble(Region = regions[1:2],
        ...
        geom_text(data = tibble(Region = regions[3:4],
...
# lineColor = "black"; fontFamily = "Passion One"; textSize = 6; retData = FALSE; seeds = mySeeds; submission = nnetSubmissionAwin; teams = myTeams; textSize = 4; retData = TRUE; logo = myLogo; regions = setNames(c("South", "East", "Midwest", "West"), c("X", "W", "Y", "Z"))
...
bracketResAwin <- bracketDraw(submission = nnetSubmissionAwin, 
            seeds = mySeeds, teams = myTeams, textSize = 3,
            retData = TRUE)
...
setwd(paste0(start_dir, tourneyYear))

# Setting file paths
stageDir <- paste0("march-machine-learning-mania-", tourneyYear, "/")

# Importing team seeds and regions data
seedsPath <- paste0(stageDir, ifelse(womens, "W", "M"), "NCAATourneySeeds.csv")
mySeeds <- read_csv(paste0(seedsPath)) %>% 
  filter(Season == tourneyYear)

# Importing model predictions
pred_submission <- readr::read_csv(paste0("Preds/", model, "/preds_nnet_", 
...
long_rounds <- make_matchups_data(seeds = mySeeds,
                                         submission = pred_submission,
                                         teams = myTeams,
                                         regions = setNames(c("South", "East", "Midwest", "West"), 
                                                            c("X", "W", "Y", "Z"))) |> 
  distinct() |> 
...
if(file.exists(paste0("sim_results_", tourneyYear, "_", model, "_N", n_sims, ".rds")) == FALSE) {
...
write_rds(sim_results, file = paste0("sim_results_", tourneyYear, "_", model, "_N", n_sims, ".rds"))

} else {
  if(file.exists(paste0("sim_summary_", tourneyYear, "_", model, "_N", n_sims, ".rds")) == FALSE) {
  # Import simulation results from saved file
  sim_results <- read_rds(paste0("sim_results_", tourneyYear, "_", model, "_N", n_sims, ".rds"))
...
write_rds(list(sim_summary = sim_summary,
          ...
          file = paste0("sim_summary_", tourneyYear, "_", model, "_N", n_sims, ".rds"))
} else {
  sim_summary_list <- read_rds(paste0("sim_summary_", tourneyYear, "_", model, "_N", n_sims, ".rds"))
  
  # Create tibbles in environment from list
list2env(sim_summary_list, envir = .GlobalEnv)
}
}
...
```{r, fig.width = 25, fig.height= 7}
plotly::ggplotly(bracketResMOV$bracketGG, tooltip = "text") |> 
  plotly::style(textposition = "right") |> 
  text_adjust_plotly(layers = c(2, 4, 5, 7), adjustment = "right") |> 
  text_adjust_plotly(layers = c(3, 8), adjustment = "left") |> 
  text_adjust_plotly(layers = 6, adjustment = "center")
...
### Team Ratings & Round Probabilities: Binary Outcome Model

```{r}
# Function to calculate power rankings for teams
power_rankings <- function(myData) {
...
rank_table <- function(myData, gradColors = c("#ffa241", "white", "#065cb5")) {
  myData <- power_rankings(myData) 
...
round_power_data <- function(summary_data, bracket_data, region_data) {
  # Adding seed and region information
round_probs <- summary_data |> left_join(bind_rows(dplyr::rename(distinct(bracket_data, SeedFav, Favorite), "Team" = "Favorite", "Seed" = "SeedFav"),                                              dplyr::rename(distinct(bracket_data, SeedDog, Underdog), "Team" = "Underdog", "Seed" = "SeedDog")) |>
  distinct() |> 
  drop_na()) |> 
  left_join(bind_rows(dplyr::rename(distinct(region_data, TeamName1, Region1), "Region" = "Region1", "Team" = "TeamName1"),                                              dplyr::rename(distinct(region_data, TeamName2, Region2), "Region" = "Region2", "Team" = "TeamName2")) |>
    distinct() |> 
    drop_na())

# Adding in power rankings and displaying table
round_power <- round_probs |> 
  left_join(power_rankings(myData = bracket_data)) |> 
...
# Function for creating reactable table
# Following example at: https://kcuilla.github.io/reactablefmtr/articles/themes.html
round_reactable_fun <- function(myData, gradColors = c("#ffa241", "white", "#065cb5")) {
  myData %>% 
  reactable(showSortIcon = FALSE,
          searchable = TRUE,
    ...
    style = color_scales(myData, colors = rev(gradColors))
          ...
          columns = list(
              Region = colDef(maxWidth = 100, rowHeader = TRUE, sticky = "left"),
              Seed = colDef(
              maxWidth = 60,
...
round_power_data(summary_data = sim_summary,
                 bracket_data = bracketResAwin$ret,
                 region_data = winners_data) |> 
  round_reactable_fun()
```
...
```{r}
rank_table(myData = bracketResMOV$ret)
```
