...
n_sims <- 10000
uniform_vals <- matrix(runif(n = n_games*n_sims, min = 0, max = 1),
                       ncol = n_sims)
...
sim_results <- tibble(Team = unlist(furrr::future_map(.x = 1:n_sims,
        .f = function(sim_num) {
          walk_tournament(rounds_data = dplyr::mutate(long_rounds,
                                            Winner = ifelse(Pred1 > uniform_vals[, sim_num], TeamName1, TeamName2),
                                            Loser = ifelse(Pred1 <= uniform_vals[, sim_num], TeamName1, TeamName2)))
        })), Round = rep(rounds, times = n_sims))
